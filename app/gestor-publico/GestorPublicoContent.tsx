...
export function GestorPublicoContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Enviar pra API do Next.js (que salva no Supabase + Google Sheets)
      ...
      <Container>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-24">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
            <div className="rounded-3xl p-8 md:p-10 border border-purple-500/15 backdrop-blur-md" style={{ background: 'rgba(124,77,255,0.03)' }}>
              {status === 'success' ? (
                <div className="text-center py-12 space-y-4">
                  ...
                  <h3 className="text-2xl font-bold">Você está na fila!</h3>
                ...
                <>
                  <h3 className="text-2xl font-bold mb-2">Solicitar acesso antecipado</h3>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      ...
                      <input required type="text" value={formData.nome} onChange={e => setFormData(s => ({ ...s, nome: e.target.value }))} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-foreground placeholder:text-muted-foreground/50 focus:border-primary/50 focus:outline-none" placeholder="João da Silva" />
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-1 block">Email corporativo</label>
                      <input required type="email" value={formData.email} onChange={e => setFormData(s => ({ ...s, email: e.target.value }))} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-foreground placeholder:text-muted-foreground/50 focus:border-primary/50 focus:outline-none" placeholder="joao@prefeitura.gov.br" />
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-1 block">Cargo</label>
                      <input required type="text" value={formData.cargo} onChange={e => setFormData(s => ({ ...s, cargo: e.target.value }))} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-foreground placeholder:text-muted-foreground/50 focus:border-primary/50 focus:outline-none" placeholder="Secretário de Gestão, Auditor, Procurador..." />
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-1 block">Órgão / Município</label>
                      <input required type="text" value={formData.orgao} onChange={e => setFormData(s => ({ ...s, orgao: e.target.value }))} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-foreground placeholder:text-muted-foreground/50 focus:border-primary/50 focus:outline-none" placeholder="Prefeitura de Niterói, TCE-RJ..." />
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-1 block">Telefone (opcional)</label>
                      <input type="tel" value={formData.telefone} onChange={e => setFormData(s => ({ ...s, telefone: e.target.value }))} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-foreground placeholder:text-muted-foreground/50 focus:border-primary/50 focus:outline-none" placeholder="(21) 9XXXX-XXXX" />
                    </div>
                    <Button type="submit" size="lg" className="w-full flex gap-2 h-14 text-lg mt-2 shadow-xl shadow-purple-500/25" disabled={status === 'loading'}>
        ...
        <div className="text-center mb-16">
          <h2 className="text-3xl font-bold tracking-tight md:text-4xl mb-4">O que o BXat faz pelo gestor público</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-24">
          {beneficios.map((b, i) => (
            <motion.div key={b.title} className="rounded-2xl p-6 border border-white/5 backdrop-blur-md" style={{ background: 'rgba(255,255,255,0.02)' }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
        ...
        {/* PROVA SOCIAL */}
        <div className="rounded-3xl p-8 md:p-12 text-center mb-24 border border-green-500/10" style={{ background: 'rgba(16,185,129,0.03)' }}>
          <p className="text-muted-foreground mb-2 text-sm">Já em uso por</p>
          <h3 className="text-4xl font-bold text-primary mb-2">CID/UFF</h3>
        ...
        {/* CTA FINAL */}
        <div className="rounded-3xl p-8 md:p-16 text-center" style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.08) 0%, rgba(124,77,255,0.05) 100%)', border: '1px solid rgba(16,185,129,0.15)' }}>
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Gestão pública não precisa ser lenta.</h2>
